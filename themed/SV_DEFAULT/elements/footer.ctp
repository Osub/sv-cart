 ...
 *===========================================================================
 * $开发: 上海实玮$
 * $Id: footer.ctp 4763 2009-09-30 02:24:51Z huangbo $
*****************************************************************************/
?>
<!-- 底部帮助 文章 文章分类 -->
...
<div class="box" <?php if($k==(sizeof($navigations_help)-1)){?>style="margin-right:0;"<?php }?>>
	...
	<ul><?php foreach($v['articles'] as $kk=>$vv){?><li><?php //echo $vv['ArticleI18n']['title'];?><?php echo $html->link($vv['ArticleI18n']['title'],$svshow->article_link($vv['Article']['id'],$dlocal,$vv['ArticleI18n']['title'],$this->data['configs']['article_link_type']),array('title'=>$vv['ArticleI18n']['title']),false,false);?></li><?php } ?></ul>
    <?php } ?>
	</div>
<?php }} ?>
<!--帮助中心显示结束-->
...
<?php if(isset($navigations_footer) && sizeof($navigations_footer)>0){?>
	...
	<p class="link"><?php foreach($navigations_footer as $k => $navigation){?><?php if($navigations_footer_num!=0){?><span>|</span><?php }?><?php echo $html->link($navigation['NavigationI18n']['name'],$navigation['NavigationI18n']['url'],array("target" =>$navigation['Navigation']['target']),false,false);?><?php $navigations_footer_num++; }?></p>
<?php }?>
