...
<?php echo $this->element('ur_here', array('cache'=>'+0 hour'));?>
	<div id="Products_box">
<h1 class="headers"><span class="l"></span><span class="r"></span><b><?php echo $promotion['PromotionI18n']['title']; ?></b></h1>
        <div id="Edit_box">
  <div id="article_info">
  ...
  <div id="user_msg">
  	<p class="article_time">
  	  <span class="title">&nbsp;<?php echo $promotion['Promotion']['start_time']." - ".$promotion['Promotion']['end_time']; ?></span></p><br />
    <p class="article">
    	<span>
    		<?php echo $promotion['PromotionI18n']['meta_description']; ?>
    	</span>
    </p>
  </div>
  <div id="select_article"> <p><?php if($neighbours['next']){ ?>
 <?php echo $SCLanguages['next'].$SCLanguages['piece'];?>: 
  <?php echo $html->link("{$neighbours['next']['PromotionI18n']['title']}","{$neighbours['next']['Promotion']['id']}",array(),false,false); } ?>
<br />
 <?php if($neighbours['prev']){?>
 <?php echo $SCLanguages['previous'].$SCLanguages['piece'];?>: 
 <?php echo $html->link("{$neighbours['prev']['PromotionI18n']['title']}","{$neighbours['prev']['Promotion']['id']}",array(),false,false); } ?>
</p></div> </div>
</div><!--文章列表End-->
<!--相关商品-->
<?php 
	...
	<?php 
		foreach($products as $key=>$v){
			...
			echo $html->tag('li',$html->para('pic',
				 ...
				 .$html->tag('span',$html->tag('font',$svshow->price_format($v['Product']['shop_price'],$SVConfigs['price_format']),array('color'=>'#F9630C')),'Price')		 		
				 ,array(),false),'');				
			}
?>
</ul>
