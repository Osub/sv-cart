...
/*        try{
    ...
    var change_remark_callback ={
     ...
     function layer_dialog_show_p(dialog_content){
		layer_dialog();
	...
	function update_cart(){
		document.forms['update_num'].submit();
	}
	
	function edit_address_no_ajax(here){
		document.forms['edit_address_act_'+here].submit();
	}
	
	function checkout_order_no_ajax(){
		document.forms['cart_info'].submit();
	}
	/*
	function select_promotion_product(pid,key,max,ch){
		document.getElementById("radios"+key).checked=true;
		var is_selected = document.getElementById("p_num"+pid).value;
		
		if(is_selected <= max*1-1){
			document.getElementById("p_num"+pid).value = is_selected*1 + 1;
		}else{
			ch.checked=false;
			layer_dialog_show_p(exceed_upper_limit_products);
	...
	function select_promotion_product(pid,key,max,ch){
		document.getElementById("p_num"+pid).value = 0;
		var i = 0;
		var num = 0;
		var checked_num = 0;
		var unchecked_num = 0;
		while(true){
			var getid = "product_radios"+pid+i;
		...
		if(checked_num > max){
			...
			document.getElementById("radios"+key).checked=false;
		}
