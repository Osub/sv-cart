...
<?php echo $html->image('protion-top.gif',array('align'=>'absbottom'))?>
...
<span class="stow cart">
	
			<?php if(isset($SVConfigs['use_ajax']) && $SVConfigs['use_ajax'] == 0){?>
				<?php echo $form->create('carts',array('action'=>'buy_now','name'=>'buy_nowpackaging'.$v['Packaging']['id'],'type'=>'POST'));?>
					<input type="hidden" name="id" value="<?php echo $v['Packaging']['id']?>"/>
					<input type="hidden" name="quantity" value="1"/>
					<input type="hidden" name="type" value="packaging"/>
					<?php echo $html->link($SCLanguages['put_into_cart'],"javascript:buy_now_no_ajax({$v['Packaging']['id']},1,'packaging')","",false,false)?>
				<?php echo $form->end();?>
			<?php }else{?>			
	
	<?php echo $html->link(__($SCLanguages['put_into_cart'],array("alt"=>$SCLanguages['put_into_cart'])),"javascript:buy_on_cart_page('packaging',".$v['Packaging']['id'].")");?>
...
<?php echo $html->image('protion-bottom.gif',array('align'=>'texttop'))?>
<?php }?>
<?php }?>