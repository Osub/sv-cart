...
<span class="stow cart">
			<?php if(isset($SVConfigs['use_ajax']) && $SVConfigs['use_ajax'] == 0){?>
				<?php echo $form->create('carts',array('action'=>'buy_now','name'=>'buy_nowcard'.$v['Card']['id'],'type'=>'POST'));?>
					<input type="hidden" name="id" value="<?php echo $v['Card']['id']?>"/>
					<input type="hidden" name="quantity" value="1"/>
					<input type="hidden" name="type" value="card"/>
					<?php echo $html->link($SCLanguages['put_into_cart'],"javascript:buy_now_no_ajax({$v['Card']['id']},1,'card')","",false,false)?>
				<?php echo $form->end();?>
			<?php }else{?>			
	
	<?php echo $html->link(__($SCLanguages['put_into_cart'],array("alt"=>$SCLanguages['put_into_cart'])),"javascript:buy_on_cart_page('card',".$v['Card']['id'].")");?>
