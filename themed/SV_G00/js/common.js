...
YAHOO.example.container.wait.setBody("<object id='loading' data='"+webroot_dir+themePath+"img/loading.swf' type='application/x-shockwave-flash' width='240' height='40'><param name='movie' value='"+webroot_dir+themePath+"img/loading.swf' /><param name='wmode' value='Opaque'></object>");
...
//执行登陆-ajax-begin
	function act_login_ajax(name,password,captcha){
		YAHOO.example.container.wait.show();
		var sUrl = webroot_dir+"user/login/";
...
// 登陆框特效开始
	...
	function show_login_captcha(id){
		document.getElementById(id).src = webroot_dir+"user/captcha/?"+Math.random();
	}	
...
//用户退出--begin
	function logout(){
		YAHOO.example.container.wait.show();
		var sUrl = webroot_dir+"user/logout/";
...
//将商品添加到购物车--end
	...
	function favorite(type_id,type){
		YAHOO.example.container.wait.show();
		var sUrl = webroot_dir+"user/favorites/add/";
...
function add_booking(){
	...
	function recommend_to_friend(id,name){
		YAHOO.example.container.wait.show();
		var sUrl = webroot_dir+"user/friends/recommend/";
		var postData ="id="+id+"&name="+name;
