...
<cite class="new_group" style="cursor:pointer;">
    <?php if(isset($SVConfigs['friend_category']) && isset($this->data['my_friends_list']) && ( sizeof($this->data['my_friends_list']) < $SVConfigs['friend_category'])){?>
    <?php echo $html->image(isset($img_style_url)?$img_style_url."/".'add-icon.gif':'add-icon.gif',array())?>
    <a href="javascript:newfriendgroup();"><?php echo $SCLanguages['add'].$SCLanguages['group']?></a>
    <?php }else if(isset($SVConfigs['friend_category']) && !isset($this->data['my_friends_list'])){?>
    <?php echo $html->image(isset($img_style_url)?$img_style_url."/".'add-icon.gif':'add-icon.gif',array())?>
    <a href="javascript:newfriendgroup();"><?php echo $SCLanguages['add'].$SCLanguages['group']?></a>
    <?php }?>
    </cite>
    </h1>
<?php if(isset($this->data['my_friends_list']) && sizeof($this->data['my_friends_list'])>0){?>
<?php foreach($this->data['my_friends_list'] as $k=>$v){?>

    <div class="infos">
    	<p class="cat_friend" ><b>·<span id="contact_cat[<?php echo $v['UserFriendCat']['id']?>]">
    	<?php echo $v['UserFriendCat']['cat_name']?></span>· <font color="red" id="cat_msg<?php echo $v['UserFriendCat']['id']?>"></font></b>
	    <?php if($v['UserFriendCat']['user_id']>0){?>
	    	<span class="add_friend">
	    		<a href="javascript:modify_cat(<?php echo $v['UserFriendCat']['id']?>)"><?php echo $SCLanguages['edit']?></a>|
	    		<a href="javascript:del_contact_cat(<?php echo $v['UserFriendCat']['id']?>,<?php echo $v['count']?>);"><?php echo $SCLanguages['delete']?></a>
	    	</span>
    	<?php }?>
    	</p>
        ...
        <span class="add_friend"><?php echo $html->image(isset($img_style_url)?$img_style_url."/".'add-icon.gif':'add-icon.gif',array("class"=>"vmiddle"))?> 
...
<form  action="" method="post" name="ContactForm<?php echo $k?>">
	...
	<ul>
		...
		<div class="y_but submits" style="padding-left:130px;">
 ...
 <?php foreach($v['user'] as $key=>$val){?>
	<ul class="friend_title friend_list" style="border:0;" onmouseover="this.className='friend_title friend_list color_e'" onmouseout="this.style.background='#ffffff'">
  	...
  	<a href="javascript:del_friends(<?php echo $val['UserFriend']['id'] ?>)"><span><?php echo $SCLanguages['delete']?></span></a></li>
</ul>
 <!--编辑好友弹出框-->
...
<form  action="" method="post" name="EditContactForm<?php echo $key?>">
	...
	<input type="hidden" name="data[UserFriend][id]" id="EditUserFriendId" value="<?php echo $val['UserFriend']['id']?>">
	<input type="hidden" name="data[UserFriend][user_id]" id="EditUserFriendUserId" value="<?php echo $user_id?>">
	<input type="hidden" name="data[UserFriend][cat_id]" id="EditUserFriendCatId" value="<?php echo $val['UserFriend']['cat_id']?>">
...
<input type="hidden" name="cat_id" value="<?php echo $v['UserFriendCat']['id'] ?>"></>
	...
	<ul>
		<li>
		<dd><?php echo $SCLanguages["friend"]?><?php echo $SCLanguages["name"]?>：</dd><dt><input size="30" type="text" name="data[UserFriend][contact_name]" id="EditUserFriendContactName<?php echo $key?>" value="<?php echo $val['UserFriend']['contact_name']?>"/>&nbsp;<font color="red" id="Editfriendname<?php echo $key?>">*</font></dt>
		</li>
		<li>
		<dd><?php echo $SCLanguages['username']?>：</dd><dt><input size="30" type="text" name="data[UserFriend][contact_user_name]" id="EditUserFriendContactUserName<?php echo $key?>" value="<?php echo $val['UserFriend']['contact_user_name']?>"/></dt>
		</li>
		<li>
		<dd><?php echo $SCLanguages['mobile']?>：</dd><dt><input size="30" type="text" name="data[UserFriend][contact_mobile]" id="EditUserFriendContactMobile<?php echo $key?>" value="<?php echo $val['UserFriend']['contact_mobile']?>"/>&nbsp;<font color="red" id="Editmobile<?php echo $key?>"></font></dt>
		</li>
		<li>
		<dd><?php echo $SCLanguages['email']?>：</dd><dt><input type="text" size="30" name="data[UserFriend][contact_email]" id="EditUserFriendContactEmail<?php echo $key?>" value="<?php echo $val['UserFriend']['contact_email']?>"/>&nbsp;<font color="red" id="Editfriendemail<?php echo $key?>"></font></dt>
		</li>
		<li>
		<dd><?php echo $SCLanguages['backup_email']?>：</dd><dt><input size="30" type="text" name="data[UserFriend][contact_other_email]" id="EditUserFriendContactOtherEmail<?php echo $key?>" value="<?php echo $val['UserFriend']['contact_other_email']?>"/>&nbsp;<font color="red" id="Editortherfriendemail<?php echo $key?>"></font></dt>
		</li>
		<li class="address">
		<dd><?php echo $SCLanguages['address']?>：</dd>
		<dt><input type="text" name="data[UserFriend][address]" id="EditUserFriendAddress<?php echo $key?>" value="<?php echo $val['UserFriend']['address']?>"/>&nbsp;<font color="red" id="Editaddress<?php echo $key?>"></font></dt>
		</li>
	</ul>
	<div class="y_but submits" style="padding-left:130px;">
