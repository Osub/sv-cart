...
<span class="Mode"><?php echo $SCLanguages['sort_by'];?>：</span><span class="Mode_img">
  ...
  <ul class="buy_products_title"><li class="sum">&nbsp;</li><li class="number"><?php echo $SCLanguages['order_code']?></li><li class="time"><?php echo $SCLanguages['purchase'].$SCLanguages['time']?></li><li class="many"><?php echo $SCLanguages['purchase'].$SCLanguages['amount']?></li><li class="handel"><?php echo $SCLanguages['operation']?></li></ul>
      ...
      <?php foreach($my_orders_products as $k=>$v){?>
		...
		<?php echo $html->link($v['CategoryI18n']['name'],$sever_host.$cart_webroot."categories/".$v['Category']['id'],array(),false,false);?>				
		<?php }?><?php if(isset($v['Brand']) && isset($v['BrandI18n'])){?>
		<?php echo $html->link($v['BrandI18n']['name'],$sever_host.$cart_webroot."brands/".$v['Brand']['id'],array(),false,false);?>				
		<?php }?></p>
				</li>
				<li class="number"><?php echo $v['OrderProduct']['order_code']?></li>
				<li class="time"><?php echo $v['OrderProduct']['created']?></li>
				<li class="many"><?php echo $svshow->price_format($v['OrderProduct']['product_price'],$SVConfigs['price_format']);?></li>
...
<?php }?></strike></span>
			...
			|
			<?php echo $html->link($SCLanguages['view'].$SCLanguages['products'],$svshow->sku_product_link($v['Product']['id'],$v['ProductI18n']['name'],$v['Product']['code'],$SVConfigs['use_sku'],$server_host,$cart_webroot),array(),false,false);?>
           	|
...
<?php }?>&nbsp;</li>
				...
				<li class="many"><font color="#ff0000"><?php echo $svshow->price_format($v['Product']['shop_price'],$SVConfigs['price_format']);?></font></li>
