...
function check_input_password(){
	...
	document.getElementById("password_loading").style.display = "";
	if (pwd.length == 0){
		document.getElementById('password_msg').innerHTML = "<font color=red>"+page_password_can_not_empty+"</font>";
	}else if(pwd.length < 6){
		document.getElementById('password_msg').innerHTML = "<font color=red>"+page_password_length_short+"</font>";
	}else{
		document.getElementById('password_msg').innerHTML = "<font color=green >√</font>";
	}
	document.getElementById('password_loading').style.display = "none";
}
...
//	check_input('password_confirm');
	...
	if(password != pwd_cfm){
		document.getElementById('password_confirm_msg').innerHTML = "<font color=red>"+page_Passwords_are_not_consistent+"</font>";
	}else{
			document.getElementById('password_confirm_msg').innerHTML = "<font color=green >√</font>";

	}
	document.getElementById("password_confirm_loading").style.display = "none";
}
...
function Personal(targetid,objN){
   ...
   var Expansions=document.getElementById(objN);
    ...
    Expansions.style.background="url("+root_all+'user/'+themePath+"img/down_icon.gif) no-repeat left";
            } else {
                Shrink.style.height="100%";
                Shrink.style.lineHeight="normal";
    Expansions.style.background="url("+root_all+'user/'+themePath+"img/up_icon.gif) no-repeat left";
            }
}