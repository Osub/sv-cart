...
<span class="left"><?=$html->image("title_l.png")?></span>
<span><?=$SCLanguages['create'].$SCLanguages['friend'].$SCLanguages['group']?></span>
  ...
  <div class="addgroup_box">
	...
	<input type="hidden" name="data[UserFriendCat][user_id]" id="UserFriendCatUserId" value="<?echo $user_id?>">
	<div class="add_froend_box">
	<br />
		<ul class="list" style="margin:0;width:auto;">
		<li>
		<dd class="l"><?=$SCLanguages['group']?><?=$SCLanguages['apellation']?>：</dd>
		<dt style="padding-top:2px;*padding-top:0;"><input type="text"  name="data[UserFriendCat][cat_name]" id="UserFriendCatCatName"/></dt>
		<dt class="submits">
		<span  class="float_l"><input style="vertical-align:top" type="button" name="Submit2" value="<?=$SCLanguages['confirm']?>"  onclick="javascript:submit_insertcat();"/></span> &nbsp;<font color="red" >*</font></dt>
		</li>
			<li>
		<dd class="l"></dd>
  	...
  	<p><?=$html->image('droup_bottom.gif',array());?></p>
</div>
<?php	
