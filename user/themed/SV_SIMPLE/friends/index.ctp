...
<cite class="new_group" style="cursor:pointer;">
    ...
    <?php echo $html->image('add-icon.png',array())?>
<?php if(isset($SVConfigs['use_ajax']) && $SVConfigs['use_ajax'] == 0){?>
    <a href="javascript:newfriendgroupnoajax();"><?php echo $SCLanguages['add'].$SCLanguages['group']?></a>
	<?php }else{?>
...
<?php }}?>
    ...
    <div id="addgroup_box" style="display:none;">
    	
  <div class="addgroup_box"  style="border:0px;">
<?php echo $form->create('friends',array('action'=>'add_cat','name'=>'InsertCat','type'=>'POST'));?>
	<input type="hidden" name="action_type" value="insert_cat">
	<input type="hidden" name="user_id" id="UserFriendCatUserId" value="<?php echo $user_id?>">
	...
	<br />
		...
		<dt style="padding-top:2px;*padding-top:0;"><input type="text"  name="cat_name" id="UserFriendCatCatName"/></dt>
		<dt class="submits">
		<span  class="float_l"><input style="vertical-align:top" type="submit" name="Submit2" value="<?php echo $SCLanguages['confirm']?>" /></span> &nbsp;<font color="red" >*</font></dt>
		</li>
			<li>
		<dd class="l"></dd>
...
<?php foreach($this->data as $k=>$v){?>
    ...
    <div id="infos">
		...
		<?php if(isset($SVConfigs['use_ajax']) && $SVConfigs['use_ajax'] == 0){?>
    	<a href="javascript:modify_cat_no_ajax(<?php echo $v['UserFriendCat']['id']?>)"><?php echo $SCLanguages['edit']?></a>
		<?php }else{?>
    	...
    	<!--  非ajax  -->
  <div class="addgroup_box" id="update_cat[<?php echo $v['UserFriendCat']['id']?>]"  style="border:0px;display:none;">
<?php echo $form->create('friends',array('action'=>'modifycat','name'=>'UpdateCat','type'=>'POST'));?>
	<input type="hidden" name="action_type" value="update_cat">
	...
	<br />
		...
		<li>
		<dd class="l"><?php echo $SCLanguages['group']?><?php echo $SCLanguages['apellation']?>：</dd>
		<dt style="padding-top:2px;*padding-top:0;"><input type="text" value="<?php echo $v['UserFriendCat']['cat_name']?>" name="cat_name" id="UserFriendCatCatName"/></dt>
		<dt class="submits">
		<span  class="float_l"><input style="vertical-align:top" type="submit" name="Submit2" value="<?php echo $SCLanguages['confirm']?>" /></span> &nbsp;<font color="red" >*</font></dt>
		</li>
			<li>
...
<?php echo $form->end();?>
        ...
        <span class="add_friend"><?php echo $html->image('add-icon.png',array("class"=>"vmiddle"))?> 
...
<?php }?>
 ...
 <?php }?>
  <?php }else{?>
	<div id="infos">
	<br /><br />
	<p style="font-size:14px;" align='center'><?php echo $html->image('warning_img.gif',array('align'=>'middle','style'=>'margin-right:15px;margin-top:-10px;'))?><?php echo $SCLanguages['no_friends'];?></p>
	<br /><br />
	</div>
  <?php }?>
</div>
