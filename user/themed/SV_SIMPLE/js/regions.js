	...
	function show_regions(str,id){
		var sUrl = webroot_dir+"regions/choice/";
		var eid = 'region_loading'+id;
		document.getElementById(eid).style.display = "";
		var postData ="str="+str+"&id="+id;
	...
	var show_regions_Success = function(o){
		...
		var eid = 'region_loading';
		var rid = 'regions';
		if(result.ad_id !="" && result.ad_id != " "){
			rid +=result.ad_id;
			eid +=result.ad_id;
		}
		document.getElementById(eid).style.display = "none";
		if(result.type == "0"){
			document.getElementById(rid).innerHTML = result.message;
		}else{
	...
	function reload_regions(id){
		...
		var str="";
		while(true){
			if(document.getElementById('AddressRegion'+id+i)==null){
				break;
			}
			str +=document.getElementById('AddressRegion'+id+i).value + " ";
			i++;
 		}
        show_regions(str,id);
	}
	
	function show_regions_by_id(str,id){
		var sUrl = webroot_dir+"regions/choice/";
		document.getElementById('region_loading'+id).style.display = "";
		var postData ="str="+str+"&id="+id;
		var request = YAHOO.util.Connect.asyncRequest('POST', sUrl, show_regions_callback,postData);
	}
