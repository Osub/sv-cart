...
function del_contact_cat(CatId,Count){
  if(Count > 0){
    layer_dialog_show('1',friends_in_group_not_cancelled,2,page_confirm,"");
  }
  else{
...
function modify_cat_no_ajax(CatId){
	var obj=document.getElementById('update_cat['+CatId+']');
...
//修改分组名称-------end
	...
	function newfriendgroupnoajax(){
		if(document.getElementById('addgroup_box').style.display == "none"){
			document.getElementById('addgroup_box').style.display = "";
		}else{
			document.getElementById('addgroup_box').style.display = "none";
		}
	}