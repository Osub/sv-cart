...
class InformationResourcesController extends AppController {
	...
	var $uses = array("InformationResource","InformationResourceI18n");
 	...
 	function index($orderby = 'created', $ordertype = 'desc'){
		/*判断权限*/
		$this->operator_privilege('information_resources_view');
		/*end*/
		$this->pageTitle = "信息库"." - ".$this->configs['shop_name'];
		$this->navigations[] = array('name'=>'系统管理','url'=>'');
		$this->navigations[] = array('name'=>'信息库','url'=>'/information_resources/');
	    ...
	    $resource = $this->InformationResource->tree($condition,"InformationResource.$orderby $ordertype",$rownum,$page);//取所有资源
//	    pr($resource);exit;
	...
	function edit($id){
		...
		$this->navigations[] = array('name'=>'系统管理','url'=>'');
		$this->navigations[] = array('name'=>'资源管理','url'=>'/information_resources/');
		...
		if($this->RequestHandler->isPost()){
			...
			$this->flash("资源 ".$userinformation_name." 编辑成功。点击这里继续编辑该菜单。",'/InformationResources/edit/'.$id,10);
	...
	function add(){
		...
		$this->operator_privilege('information_resources_view');
		/*end*/		
		$this->pageTitle = "添加资源 - 资源管理"." - ".$this->configs['shop_name'];
		$this->navigations[] = array('name'=>'系统管理','url'=>'');
		$this->navigations[] = array('name'=>'资源管理','url'=>'/information_resources/');
		$this->navigations[] = array('name'=>'新增资源','url'=>'');
		$this->InformationResource->set_locale($this->locale);
		$this->set('navigations',$this->navigations);
		...
		//	pr($this->data['InformationResource']);exit;
			...
			$this->flash("资源 ".$userinformation_name."  添加成功。点击这里继续编辑该资源。",'/information_resources/edit/'.$id,10);
		}
		$this->InformationResource->set_locale($this->locale);
	...
	function remove($id){
		...
		if($result)
			$this->flash('删除失败，改资源还有子资源','/InformationResources/','');
		else{
			$this->InformationResource->del($id);
			//操作员日志
  //  	    if(isset($this->configs['open_operator_log']) && $this->configs['open_operator_log'] == 1){
 //   	    $this->log('操作员'.$_SESSION['Operator_Info']['Operator']['name'].' '.'删除系统资源:'.$system_info['InformationResourceI18n']['name'] ,'operation');
  //  	    }
