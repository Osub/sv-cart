 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: product_downloads_controller.php 4728 2009-09-29 03:50:40Z huangbo $
 *****************************************************************************/
class ProductDownloadsController extends AppController {
	...
	var $uses = array('ProductLocalePrice','ProductGalleryI18n','Order',"Shipping","ProdcutVolume","Stock","ProviderProduct",'Config',"SystemResource","OrderProduct","ProductDownload","ProductRank","TopicProduct","Product","UserRank","Category","Brand","Provider","BookingProduct","ProductType","ProductGallery","ProductRelation","ProductArticle","ProductI18n","ProductTypeAttribute","ProductAttribute","ProductsCategory","BrandI18n",'SeoKeyword');
	...
	function add(){
        ...
        if($this->RequestHandler->isPost()){
          ...
          $id=$this->Product->id;
            ...
            $this->flash("商品 ".$product_code." ".$product_name." 添加成功。点击这里继续编辑该商品。",'/product_downloads/'.$id,10);
        }
        $categories_tree=$this->Category->tree('P',$this->locale);
        $brands_tree=$this->Brand->getbrandformat();
    ...
    function view($id)
    {
        ...
        if($this->RequestHandler->isPost()){
            ...
            if(isset($this->params['form']['action_type']) && $this->params['form']['action_type']=='product_base_info'){
                ...
                $this->flash("商品 ".$product_code." ".$product_name." 编辑成功。点击这里继续编辑该商品。",'/product_downloads/'.$id,10);
            }
            //编辑相册
            if(isset($this->params['form']['action_type']) && $this->params['form']['action_type']=='product_gallery'){
                ...
                $this->flash("商品 ".$product_code." ".$product_name." 编辑相册成功。点击这里继续编辑该商品。",'/product_downloads/'.$id,10);
            }
            //商品属性编辑
            if(isset($this->params['form']['action_type']) && $this->params['form']['action_type']=='product_attr'){
                ...
                $this->flash("商品 ".$product_code." ".$product_name." 属性编辑成功。点击这里继续编辑该商品。",'/product_downloads/'.$id,10);
            }
        }
        $categories_tree=$this->Category->tree('P',$this->locale);
        //品牌
