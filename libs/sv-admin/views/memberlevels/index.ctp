...
<?php if(isset($memberlevel_list) && sizeof($memberlevel_list)>0){
 ...
 foreach($memberlevel_list as $k=>$v){ ?>
	...
	<td><span><?php echo $v['UserRankI18n']['name'] ?></span></td>
	...
	<td align="center">
		<?php echo $html->link("编辑","/memberlevels/edit/".$v['UserRank']['id'],array());?> | <?php echo $html->link("移除","javascript:;",array("onclick"=>"layer_dialog_show('确定删除?','{$admin_webroot}memberlevels/remove/{$v['UserRank']['id']}')"));?>
	</td></tr>
