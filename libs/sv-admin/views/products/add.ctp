 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: add.ctp 5439 2009-10-26 10:10:48Z huangbo $
*****************************************************************************/
?>
<!--Main Start-->
...
<div class="home_main">
<ul class="tab">
	<li class="hover" id="tabs1" onclick="overtab('tabs',1,4)"><span>基本信息</span></li>
	<li class="normal" id="tabs2" onclick="overtab('tabs',2,4)"><span>详细信息</span></li>
	<li class="normal" id="tabs3" onclick="overtab('tabs',3,4)"><span>商品相册</span></li>
	<li class="normal" id="tabs4" onclick="overtab('tabs',4,4)"><span>商品属性</span></li>
</ul>
...
<?php  if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><textarea id="ProductI18n<?php echo $k;?>MetaDescription" name="data[ProductI18n][<?php echo $k;?>][meta_description]" ></textarea></span></p>
<?php 
	}
}?>
		<h2>商品网站网址：</h2>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  <div class="box">
		...
		<input type="hidden" id="ProductCode"  value="<?php echo $_SESSION['Operator_Info']['Operator']['id'];?>_temp"/>
...
<?php }}}}}}?>
	...
	<select id="ProductproviderId" name="data[Product][provider_id]">
		...
		<dd>
			...
			<?php }?><?php }else{?>
				<tr>
				<td><a href="javascript:;" name="1" onclick="addprovider(this)">[+]</a>
				<select  name="other_provider[]" style='margin-bottom:2px;'>
			    <option value="0">请选择....</option>
			    <?php if(isset($provider_list) && sizeof($provider_list>provider_list)>0){?>
			    <?php foreach($provider_list as $k=>$v){?>
	...
	<?php }?>
		...
		<!--销售量-->
		<dl><dt>销售量: </dt><dd><input type="text" style="width:150px;border:1px solid #649776" name="data[Product][sale_stat]"  /> </dd></dl>
		...
		<dl  style="*padding:5px 0;" ><dt style="padding-top:1px">是否显示款号颜色图：</dt><dd class="best_input"><input type="radio" class="checkbox" name="data[Product][is_colors_gallery]" value="1"  />是<input type="radio" class="checkbox" name="data[Product][is_colors_gallery]" value="0"  checked />否</label></dd></dl>
...
</div><div id="con_tabs_3" class="none">
<!--Product Photos-->
	<div class="order_stat properies">
...
<div class="order_stat properies">
	...
	<div class="box">
		<table>
		<tr>默认图片地址：<input type="text" name="img_url_default" style="width:270px;border:1px solid #649776"  id="upload_img_text_99999"  /><?php echo $html->link($html->image('select_img.gif',array("class"=>"vmiddle icons","height"=>"20")),"javascript:img_sel('99999','products')",'',false,false)?></td></tr>
		<tr><td><?php echo $html->image("temp",array('id'=>"logo_thumb_img_99999",'align'=>'absmiddle','width'=>'108','height'=>'108',"style"=>"display:none"));?></td></tr>
		</table>
	</div>
...
<?php $product_type_list=$this->requestAction("/commons/product_type_list/0");?>
	...
	<div class="order_stat properies">
      ...
      <input id="ProductId" name="data[Product][id]" type="hidden" >
	  <div class="properies_left">
	  
		<dl><dt>&nbsp;</dt><dd>
		<select name="product_type" id="product_type" onchange="getAttrList(0)">
		  <option value="">请选择</option>
          <?php echo $product_type_list;?>
		</select></dd></dl>

		<span id="productsAttr"></span>
		</div>
	  </div>
	</div><?php }?>
