...
<? if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?=$html->image($v['Language']['img01'])?><span><input id="product_name_<?=$v['Language']['locale']?>" name="data[ProductI18n][<?=$k;?>][name]"  type="text" maxlength="100"  style="" /></span>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  其他信息</h1></div>
	  <div class="box">
		<dl><dt><?=$html->image('help_icon.gif',array('align'=>'absmiddle'))?>商品货号：</dt><dd><input type="text" class="text_inputs"  name="data[Product][code]" value=""/><br />如果您不输入商品货号，系统将自动生成一个唯一货号。</dd></dl>
...
<?}}}}}}?>
	...
	<?}?>
		...
		<dl><dt>商品重量：</dt><dd><input type="text" class="text_inputs" value="" name="data[Product][weight]" id="ProductWeight"/>
			...
			<dl style="*padding:4px 0;"><dt style="padding-top:1px">默认库存：</dt><dd class="best_input"><input name="data[Product][quantity]" id="ProductQuantity" value="<?echo $SVConfigs['default_stock']?>"/></dd></dl>
