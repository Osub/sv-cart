...
<p class="add_categories"><strong><?php echo $html->link($html->image('add.gif',array('align'=>'absmiddle'))."商品列表","/".$_SESSION['cart_back_url'],'',false,false);?></strong></p>
...
<?php  if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><input  id="product_name_<?php echo $v['Language']['locale']?>" name="data[ProductI18n][<?php echo $k;?>][name]" style="color:<?php echo $products_name_color;?>;width:215px;" type="text" maxlength="100"  <?php if(isset($this->data['ProductI18n'][$v['Language']['locale']])){?>value="<?php echo  $this->data['ProductI18n'][$v['Language']['locale']]['name'];?>"<?php }else{?>value=""<?php }?> /></span>
...
<div id="add_price">
<?php foreach($prodcut_volume as $k=>$v){ if($k=="0"){?><dl><dt>优惠价格x：</dt><dd style="font-family:'宋体';padding-top:3px !important;">[<?php echo $html->link("+","javascript:;",array("onclick"=>"add_price()"),false,false);?>]优惠数量<input type="text" class="text_inputs" name="data[ProdcutVolume][number][]" onblur="check_number(this)" value="<?php echo $v['ProdcutVolume']['volume_number']?>" style="width:80px;" /> 优惠价格<input type="text" class="text_inputs" name="data[ProdcutVolume][price][]" onblur="check_price(this)" value="<?php echo $v['ProdcutVolume']['volume_price']?>" style="width:80px" /></dd></dl>
...
<?php }else{?>
	...
	<div id="add_price"></div>
		...
		<dl  style="*padding:5px 0;" ><dt style="padding-top:1px">是否显示款号颜色图：</dt><dd class="best_input"><label for="ProductAlone"><input type="radio" class="checkbox" name="data[Product][is_colors_gallery]" value="1" <?php if($this->data['Product']['is_colors_gallery'] == 1){ ?>checked<?php } ?> />是<input type="radio" class="checkbox" name="data[Product][is_colors_gallery]" value="0" <?php if($this->data['Product']['is_colors_gallery'] == 0){ ?>checked<?php } ?>  />否</label></dd></dl>
...
<?php echo $html->link($html->image('select_img.gif',array("class"=>"vmiddle icons","height"=>"20")),"javascript:img_sel(1,'products')",'',false,false)?>
			</span>
			</span>	</p>
			<p class="add_photo">
			图片描述 : 
<?php  if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		  <?php echo $html->image($v['Language']['img01'],array("class"=>"vmiddle"))?> <input type="text" name="img_desc[<?php echo $v['Language']['locale']?>][1]" id="img_desc[1]"/>
<?php }}?>
            	...
            	<?php if(isset($product_articles) && sizeof($product_articles)>0){?>
                      <?php foreach($product_articles as $k=>$v){?>
                          <?php if (isset($v['Article'])){?>
                           <p class="rel_list">
                           <span class="handle">
						   排序:<input size="2" value="<?php echo $v['ProductArticle']['orderby']?>" onblur="update_orderby(<?php echo $v['ProductArticle']['id']?>,this.value,'PA');">
