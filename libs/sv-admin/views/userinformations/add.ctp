...
<div class="home_main">
<?php echo $form->create('userinformations',array('action'=>'add','onsubmit'=>'return userinformations_check();'));?>
<table width="100%" cellpadding="0" cellspacing="0" class="">
<tr>
<td  align="left" width="50%" valign="top" style="padding-right:5px">
<!--Communication Stat-->

<input type="hidden" name="data[UserInfo][id]"  >
	<div class="order_stat athe_infos department_config">
	  ...
	  <br />
		...
		<dl><dt class="config_lang"><?=$html->image($v['Language']['img01'])?></dt><dd><input type="text" class="text_inputs" style="width:320px;" id="name<?=$v['Language']['locale']?>" name="data[UserInfoI18n][<?=$k;?>][name]" /> <font color="#ff0000">*</font></dd></dl>
		<?}}?>	
		<dl><dt class="config_lang">提示信息:</dt><dd></dd></dl>
		<? if(isset($languages) && sizeof($languages)>0){foreach ($languages as $k => $v){?>		
		<dl><dt class="config_lang"><?=$html->image($v['Language']['img01'])?></dt><dd><input type="text" class="text_inputs" style="width:320px;" name="data[UserInfoI18n][<?=$k;?>][message]"  /></dd></dl>
		<?}}?>	
...
<?if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<dl><dt class="config_lang"><?=$html->image($v['Language']['img01'])?></dt><dd><textarea  name="data[UserInfoI18n][<?=$k;?>][values]" ></textarea></dd></dl>
<?}}?>	
   	  </div>
	  </div>
	</div>
...
<!--Password-->
	<div class="order_stat athe_infos">
	  ...
	  <div class="box">
		<br />
		<dl><dt>是否有效：</dt>
			<dd><input type="radio" value="1" name="data[UserInfo][status]" checked /> 是 <input type="radio" name="data[UserInfo][status]" value="0" /> 否</dd></dl>
		<dl><dt>前台是否显示：</dt>
			<dd><input type="radio" name="data[UserInfo][front]" value="1" checked /> 是 <input type="radio" name="data[UserInfo][front]" value="0"   /> 否</dd></dl>
		<dl><dt>后台是否显示：</dt>
			<dd><input type="radio" name="data[UserInfo][backend]" value="1"checked /> 是 <input type="radio" name="data[UserInfo][backend]" value="0" /> 否</dd></dl>
		<dl><dt>排序：</dt>
			<dd><input type="text" name="data[UserInfo][orderby]" onkeyup="check_input_num(this)" ><font color="#646464"><br />如果您不输入排序号，系统将默认为50</font></dd></dl>
