 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: add.ctp 4482 2009-09-24 03:35:50Z huangbo $
*****************************************************************************/
?>
...
<!--Communication Stat-->
	...
	<?php //pr($languages);pr($this->data)?>
	  ...
	  <br />
  	    <dl><dt>用户名：</dt>
			<dd><input type="text" class="text_inputs" style="width:270px;" id="OperatorName" name="data[Operator][name]" value="<?php echo  $this->data['Operator']['name'];?>"/> <font color="#F94671">*</font></dd></dl>
		<dl><dt>Email地址：</dt>
			<dd><input type="text" class="text_inputs" style="width:270px;" id="OperatorEmail" name="data[Operator][email]" value="<?php echo  $this->data['Operator']['email'];?>"/> <font color="#F94671">*</font></dd></dl>
		<dl><dt>手机：</dt>
			<dd><input type="text" class="text_inputs" style="width:270px;" id="Operatormobile" name="data[Operator][mobile]" value="<?php echo  $this->data['Operator']['mobile'];?>"/> <font color="#F94671">*</font></dd></dl>
		<dl><dt>所属部门：</dt><dd>
...
<dl style="padding:5px 0;*padding:6px 0;"><dt style="padding-top:1px">状态：</dt><dd class="best_input"><input id="OperatorStatus" name="data[Operator][status]" type="radio" value="1" checked >有效<input id="OperatorStatus" name="data[Operator][status]" type="radio" value="0" >无效<input id="OperatorStatus" name="data[Operator][status]" type="radio" value="3" >冻结</dd></dl>
		...
		<?php foreach( $config_timezone as $k=>$v ){$str = explode(":",$v);?>
			<option value="<?php echo $str[0];?>" <?php if($str[0]=="-8"){ echo "selected";} ?>><?php echo $str[1];?></option>
		<?php }?>
		</select>
		</dd>
		</dl>
...
<!--Categories List-->
	...
	<div class="order_stat operators properies">
	  ...
	  <div class="box">
		...
		<span><input type="checkbox" name="operator_role[]" value="<?php echo $ov['OperatorRole']['id']?>" onclick="javascript:check('<?php if(isset($ov['OperatorRole']['actions'])){ echo $ov['OperatorRole']['actions'];}?>',this,'operator_action')"  /><?php echo $ov['OperatorRole']['name']?></span>
		<?php }}?></td>
		</tr>
		...
		<tr class="2">
			...
			<table width="100%" cellpadding="0" cellspacing="0" class="" border=1 style="border: medium none ;padding-left: 30px; border-collapse: collapse; font-family: 宋体;border-color:#C4F9A4;">
				...
				<table width="100%" cellpadding="0" cellspacing="0" class=""  >
					...
					<td width="20%">
					<input type="checkbox" name="operator_action[]" value="<?php echo $vvv['Operator_action']['id']?>"  /><?php echo $vvv['Operator_action']['name']?>
					</td>
