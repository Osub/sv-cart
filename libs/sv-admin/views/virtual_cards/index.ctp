...
<?php foreach($products_list as $k=>$v){?>	
	...
	<td align="center">
	<?php echo $html->link("查看",$server_host.$cart_webroot."products/{$v['Product']['id']}",array('target'=>'_blank'));?>|<?php echo $html->link("编辑","/virtual_cards/{$v['Product']['id']}");?>|<?php echo $html->link("复制","/virtual_cards/copy_pro/{$v['Product']['id']}");?>|<?php echo $html->link("放入回收站","javascript:;",array("onclick"=>"layer_dialog_show('确定进回收站?','{$admin_webroot}products/trash/{$v['Product']['id']}')"));?>|<?php echo $html->link("查看虚拟卡信息","/virtual_cards/card/{$v['Product']['id']}");?>|<?php echo $html->link("补货","/virtual_cards/card_add/{$v['Product']['id']}");?>|<?php echo $html->link("批量补货","/virtual_cards/batch_card_add/{$v['Product']['id']}");?>
	</td></tr>
<?php }}?>
  </table></div>
...
function export_act(){ 
	...
	document.SearchForm.action=webroot_dir+"virtual_cards/index/export/"+csv_export_code.value;
	document.SearchForm.onsubmit= "";
	document.SearchForm.submit(); 
}
</script>