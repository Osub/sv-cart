...
<p class="add_categories"><strong><?php echo $html->link($html->image('add.gif',array('align'=>'absmiddle'))."虚拟卡列表","/".$_SESSION['cart_back_url'],'',false,false);?></strong></p>
...
<!--Main Start-->
<div class="home_main">
<?php echo $form->create('virtual_cards',array('action'=>$this->data['Product']['id'],"OnSubmit"=>'return virtual_cards_check();'));?>
...
<?php if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><input id="virtual_cards_name_<?php echo $v['Language']['locale']?>" name="data[ProductI18n][<?php echo $k;?>][name]" style="color:<?php echo $products_name_color;?>;width:215px;" type="text" maxlength="100"  <?php if(isset($this->data['ProductI18n'][$v['Language']['locale']])){?>value="<?php echo  $this->data['ProductI18n'][$v['Language']['locale']]['name'];?>"<?php }else{?>value=""<?php }?> /></span>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  其他信息</h1></div>
	  <div class="box">
		<dl><dt><?php echo $html->image('help_icon.gif',array('align'=>'absmiddle',"onclick"=>"help_show_or_hide('help_text2')"))?>商品货号：</dt><dd><input type="text" class="text_inputs" id="ProductCode" name="data[Product][code]"<?php if(isset($this->data['Product']['code'])){?>value="<?php echo  $this->data['Product']['code'];?>"<?php }else{?>value=""<?php }?>/><br /><span style="display:none" id="help_text2">如果您不输入商品货号，系统将自动生成一个唯一货号。</span></dd></dl>
		
		<dl><dt>商品分类：</dt><dd><select id="ProductsCategory" name="data[ProductsCategory][category_id]">
...
<input type="checkbox" id="is_default_rank<?php echo $k?>" name="is_default_rank[<?php echo $k?>]" <?php if(@$v['UserRank']['is_default_rank']==1){ echo "checked"; }?> value="1" onclick="user_prince_check(this.checked,<?php echo $k?>)" >自动按照比例结算
	...
	<?php }?>
		...
		<!--<dl><dt>商品重量：</dt><dd><input type="text" class="text_inputs" value="<?php echo $this->data['Product']['products_weight_by_unit']?>" name="data[Product][weight]" id="ProductWeight" />
			...
			-->
			<dl style="*padding:4px 0;"><dt style="padding-top:1px">默认库存：</dt><dd class="best_input"><input class="text_inputs" value="<?php echo $this->data['Product']['quantity'];?>" name="data[Product][quantity]" id="ProductQuantity" readonly /></dd></dl>
	...
	<?php }}?>
		...
		<table width="100%" class="relative_product">
			<tr>
				...
				<p><select name="source_select1" id="source_select1" size="20" style="width:100%" ondblclick="addItem(this,'insert_link_products', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'P');"multiple="true"></p>
				</td>
				...
				<p class="direction">
				<input type="button" class="pointer" value=">" onclick="addItem(this.form.elements['source_select1'],'insert_link_products', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'P');"/><br /><br />
				</p>
				</td>
				<td valign="top" width="40%">
				...
				<?php if(isset($product_relations) && sizeof($product_relations)>0){?>
                      <?php foreach($product_relations as $k=>$v){?>
                           ...
                           <p class="rel_list">
                           <span class="handle">
                           排序:<input size="2" value="<?php echo $v['ProductRelation']['orderby']?>" onblur="update_orderby(<?php echo $v['ProductRelation']['id']?>,this.value,'P');">
                           <input type="button" class="pointer" value="删除" onclick="dropItem(<?php echo $v['ProductRelation']['related_product_id']?>,'drop_link_products', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'P');"/></span><?php echo $v['ProductI18n']['name']?>
                            <?php }?>
                      <?php }}?>
                      </div>
               </div></td>
...
<?php echo $form->create('',array('action'=>'','name'=>"articleForm","id"=>"articleForm"));?>
		...
		<select name="article_cat" id="article_cat">
		  <option value="0">文章分类</option>
		    ...
		    <?php foreach($article_cat as $k=>$v){?>
		      <option value="<?php echo $v['CategoryI18n']['name']?>"><?php echo $v['CategoryI18n']['name']?></option>
		    <?php }}?>
		...
		<table cellspacing="0" cellpadding="0" width="100%" class="relative_product">
			<tr>
				<td valign="top" width="40%">
				<p><strong>可选商品</strong></p>
				<p><select name="source_select2" id="source_select2" size="20" style="width:100%" ondblclick="addItem(this,'insert_product_articles', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'A');"multiple="true"></p>
				</td>
				<td valign="top" width="12%" align="center">
				<p><strong>操作</strong></p>
				<p class="relative_radio">
				...
				<label><input type="radio"  name="is_single" id="is_single" value="0"/>双向关联</label></p>
				<p class="direction">
				<input type="button" class="pointer" value=">" onclick="addItem(this.form.elements['source_select2'],'insert_product_articles', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'A');"/><br /><br />
				</p>
				</td>
				<td valign="top" width="40%">
            ...
            <?php if(isset($product_articles) && sizeof($product_articles)>0){?>
                      <?php foreach($product_articles as $k=>$v){?>
                          <?php if (isset($v['Article'])){?>
                           ...
                           <input type="button" class="pointer" value="删除" onclick="dropItem(<?php echo $v['ProductArticle']['article_id']?>,'drop_product_articles', <?php echo $this->data['Product']['id']?>, this.form.elements['is_single'][0].checked,'P');"/>
                           </span>
...
function font_color_picker(color_sn){
	for( var i=0; i<lang_all.length;i++){
		document.getElementById('virtual_cards_name_'+lang_all[i]).style.color=color_sn;
	}
}
onload = function(){
