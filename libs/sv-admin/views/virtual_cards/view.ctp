 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: view.ctp 622 2009-04-14 11:06:49Z huangbo $
*****************************************************************************/
//pr($this->data);
?>
...
<? if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?=$html->image($v['Language']['img01'])?><span><input id="virtual_cards_name_<?=$v['Language']['locale']?>" name="data[ProductI18n][<?=$k;?>][name]" style="color:<?echo $products_name_color;?>;width:215px;" type="text" maxlength="100"  <?if(isset($this->data['ProductI18n'][$v['Language']['locale']])){?>value="<?= $this->data['ProductI18n'][$v['Language']['locale']]['name'];?>"<?}else{?>value=""<?}?> /></span>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  其他信息</h1></div>
	  <div class="box">
		<dl><dt><?=$html->image('help_icon.gif',array('align'=>'absmiddle'))?>商品货号：</dt><dd><input type="text" class="text_inputs" id="ProductCode" name="data[Product][code]"<?if(isset($this->data['Product']['code'])){?>value="<?= $this->data['Product']['code'];?>"<?}else{?>value=""<?}?>/><br />如果您不输入商品货号，系统将自动生成一个唯一货号。</dd></dl>
...
<input type="checkbox" id="is_default_rank<?=$k?>" name="is_default_rank[<?=$k?>]" <?if(@$v['UserRank']['is_default_rank']==1){ echo "checked"; }?> value="1" onclick="user_prince_check(this.checked,<?=$k?>)" >自动按照比例结算
	...
	<?}?>
		...
		<!--<dl><dt>商品重量：</dt><dd><input type="text" class="text_inputs" value="<?echo $this->data['Product']['products_weight_by_unit']?>" name="data[Product][weight]" id="ProductWeight" />
			...
			-->
			<dl style="*padding:4px 0;"><dt style="padding-top:1px">默认库存：</dt><dd class="best_input"><input class="text_inputs" value="<?echo $this->data['Product']['quantity'];?>" name="data[Product][quantity]" id="ProductQuantity" readonly /></dd></dl>
