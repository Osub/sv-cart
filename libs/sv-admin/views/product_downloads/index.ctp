...
<?php echo $form->create('',array('action'=>'/',"type"=>"get",'name'=>"SearchForm"));?>
	...
	<dd><p>
	价格区间：<input name="min_price" id="min_price" value="<?php echo $min_price?>" style="width:50px;border:1px solid #649776" />－<input name="max_price" id="max_price" value="<?php echo $max_price?>" style="width:50px;border:1px solid #649776" />
	关键字：<input type="text" style="width:80px;border:1px solid #649776" name="keywords" id="keywords" value="<?php echo $keywords?>"/>
	添加时间：<input type="text" id="date"  style="width:80px;border:1px solid #649776" name="date" value="<?php echo @$date?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show","class"=>"calendar"))?>
		<input type="text" id="date2" style="width:80px;border:1px solid #649776" name="date2" value="<?php echo @$date2?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show2","class"=>"calendar"))?>
	有效时间：<input type="text" id="date3" style="width:80px;border:1px solid #649776" name="date3" value="<?php echo @$date3?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show3","class"=>"calendar"))?>
	<input type="text" id="date4" style="width:80px;border:1px solid #649776" name="date4" value="<?php echo @$date4?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show4","class"=>"calendar"))?>
	...
	<select style="width:100px;"  name="provider_id">
...
<p class="add_categories"><strong><?php echo $html->link($html->image('add.gif',array('align'=>'absmiddle'))."新增下载商品","add/",'',false,false);?></strong></p>
<!--Main Start-->
<!--Main Start-->
...
<?php foreach($products_list as $k=>$v){?>	
	...
	<td align="center"><?php if($v['ProductDownload']['status'] == 1){?><?php echo $html->image('yes.gif')?><?php }else{?><?php echo $html->image('no.gif')?><?php }?></td>
	<td align="center">
	<?php echo $html->link("查看","../../products/{$v['Product']['id']}",array('target'=>'_blank'));?>
	|<?php echo $html->link("编辑","/product_downloads/{$v['Product']['id']}");?>
	|<?php echo $html->link("复制","/product_downloads/copy_pro/{$v['Product']['id']}");?>
	|<?php echo $html->link("放入回收站","javascript:;",array("onclick"=>"layer_dialog_show('确定进回收站?','{$admin_webroot}product_downloads/trash/{$v['Product']['id']}')"));?>
	</td></tr>
<?php }}?>
  </table></div>
...
{ 
	document.ProForm.action=webroot_dir+"product_downloads/batch";
	document.ProForm.onsubmit= "";
	document.ProForm.submit(); 
}

function search_act(){ 
	document.SearchForm.action=webroot_dir+"product_downloads/index";
...
function export_act(){ 
	...
	document.SearchForm.action=webroot_dir+"product_downloads/index/export/"+csv_export_code.value;
	document.SearchForm.onsubmit= "";
	document.SearchForm.submit(); 
}
