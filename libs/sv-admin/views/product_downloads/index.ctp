...
<?php echo $form->create('',array('action'=>'/',"type"=>"get",'name'=>"SearchForm"));?>
	...
	有效时间：<input type="text" id="date3" name="date3" value="<?php echo @$date3?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show3","class"=>"calendar"))?>
	<input type="text" id="date4" name="date4" value="<?php echo @$date4?>"  readonly/><?php echo $html->image("calendar.gif",array('width'=>'18','height'=>'18','alt'=>'Calendar',"id"=>"show4","class"=>"calendar"))?>
...
<?php foreach($products_list as $k=>$v){?>	
	...
	|<?php echo $html->link("放入回收站","javascript:;",array("onclick"=>"layer_dialog_show('确定进回收站?','{$this->webroot}product_downloads/trash/{$v['Product']['id']}')"));?>
	</td></tr>
<?php }}?>
  </table>
...
<?php if($total>0){?><input type="submit" id="submit_button" value="放入回收站" onclick="operate_select()"/><?php }?></p>
...
//	window.location.href=webroot_dir+url;
	document.SearchForm.action=webroot_dir+"product_downloads/index/export";
	document.SearchForm.onsubmit= "";
	document.SearchForm.submit(); 
}
