...
<input type="checkbox" id="is_default_rank<?php echo $k?>" name="is_default_rank[<?php echo $k?>]" <?php if(@$v['UserRank']['is_default_rank']==1){ echo "checked"; }?> value="1" onclick="user_prince_check(this.checked,<?php echo $k?>)" >自动按照比例结算
	...
	<?php }?>
		...
		<dl><dt>促销日期：</dt><dd class="time"><input style="width:90px;" class="text_inputs" type="text" id="date" name="date" value="<?php echo $this->data['Product']['promotion_start']?>"  readonly/><?php echo $html->image("calendar.gif",array("id"=>"show","class"=>"calendar"))?>
			<input type="text" style="width:90px;" class="text_inputs" id="date2" name="date2" value="<?php echo $this->data['Product']['promotion_end']?>"  readonly/><?php echo $html->image("calendar.gif",array("id"=>"show2","class"=>"calendar"))?></dd></dl>
