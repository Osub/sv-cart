...
<!--Main Start-->
<?php echo $form->create('Role',array('action'=>'edit/'.$operatorrole['OperatorRole']['id']));?>
<div class="home_main">
...
<?php foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><input type="text" style="width:195px;" id="name<?php echo $v['Language']['locale']?>" name="data[OperatorRoleI18n][<?php echo $k;?>][name]" value="<?php echo $operatorrole['OperatorRoleI18n'][$k]['name']?>" /> <font color="#F90046">*</font></span></p>
	...
	<div class="order_stat athe_infos tongxun">
	  ...
	  <div class="box">
		...
		<dl style="margin-bottom:5px;padding-top:8px;"><dt>角色编号：</dt>
			...
			<dd>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $operatorrole['OperatorRole']['orderby']?><input type="hidden" class="text_inputs" style="width:190px;" name="data[OperatorRole][orderby]" value="<?php echo $operatorrole['OperatorRole']['orderby']?>" /><br /> 如果您不输入排序号，系统将默认为50</dd></dl>
		<dl><dt>是否有效：</dt>
...
<?php  foreach($operatoraction as $k=>$v){ ?>
	...
	<?php  if(isset($v['SubAction']) && sizeof($v['SubAction'])>0){foreach($v['SubAction'] as $kk=>$vv){  ?>
		<li><label><input type="checkbox" name="competence[]" value="<?php echo $vv['Operator_action']['id'];?>" <?php if( in_array($vv['Operator_action']['id'],$actions_arr) ){echo "checked";}?> /><?php echo $vv['Operator_action']['name'];?></label></li>
	<?php }} ?>
		</ul>
		</div>
<?php }} ?>		
