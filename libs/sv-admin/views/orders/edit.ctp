...
<!--BaseInfo-->
	<div class="order_stat athe_infos order_base">
	  ...
	  <br />
		...
		<dl><dt>购货人：</dt><dd><font face="arial"><?php echo $order_info['Order']['consignee']?></font>[<?php echo $html->link("显示购货人信息","/users/{$order_info['Order']['user_id']}",array("class"=>"none"),false,false);?>] [<?php echo $html->link("发送/查看留言","/messages/index/{$order_info['Order']['user_id']}",array("class"=>"none"),false,false);?>]</dd></dl>
  	    <dl><dt>支付方式：</dt><dd>
...
<span style="display:none" id="Payment_edit_id">
  	<?php if(isset($payment_list) && sizeof($payment_list)>0){?>
  	<?php foreach($payment_list as $k=>$v){?>
		<p class="langs"><input class="radio" type="radio" style="width:auto;border:0;" value="<?php echo $v['Payment']['id']?>" name="data[Order][payment_id]" <?php if($order_info['Order']['payment_id'] == $v['Payment']['id']){?>checked<?php }?>/><?php echo $v['PaymentI18n']['name']?></p>
		<p class="alt"><?php echo $v['PaymentI18n']['description']?></p>
	<?php }}?>
</span>
  	 <dl><dt>配送方式：</dt>
  	<dd>
...
<span style="display:none" id="Shipping_edit_id">
  	<?php if(isset($shipping_list) && sizeof($shipping_list)>0){?>
	<?php foreach($shipping_list as $k=>$v){?>
		<p class="langs"><span>￥<?php echo $v['Shipping']['shipping_fee']?></span><input type="radio" class="radio" style="width:auto;border:0;" value="<?php echo $v['Shipping']['id']?>" name="data[Order][shipping_id]" <?php if($order_info['Order']['shipping_id'] == $v['Shipping']['id']){?>checked<?php }?> /><?php echo $v['ShippingI18n']['name']?></p>
		<p class="alt"><?php echo $v['ShippingI18n']['description']?></p>
	<?php }}?>
</span>
		...
		<?}?>
		<dl><dt>订单状态：</dt><dd><font face="arial">
		...
		<dl><dt>付款时间：</dt><dd><font face="arial"><?php if($order_info['Order']['payment_status'] == 1 || $order_info['Order']['payment_status'] == 2){?><?php echo $order_info['Order']['payment_time']?><?php }else{?>未付款<?php }?></font></dd></dl>
		<dl><dt>发货时间：</dt><dd><font face="arial"><?php if($order_info['Order']['shipping_status'] == 1 || $order_info['Order']['shipping_status'] == 2){?><?php echo $order_info['Order']['shipping_time']?><?php }else{?>未付款<?php }?></font></dd></dl>
		
		<dl><dt>订单来源：</dt><dd><font face="arial"><?php echo $order_info['Order']['referer']?></font></dd></dl>
<?php //if(isset($SVConfigs['mlti_currency_module']) && $SVConfigs['mlti_currency_module'] == 1){?>
		<dl><dt>订单语言：</dt><dd>
		<font face="arial">
<?php if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<?php if($v["Language"]["locale"] == $order_info['Order']['order_locale']){?>
			<?php echo $v["Language"]["name"]?>
		<?php }}}?>
		</font></dd></dl>
		<dl><dt>订单货币：</dt><dd><font face="arial"><?php echo !empty($order_info['Order']['order_currency'])?$order_info['Order']['order_currency']:"RMB"?></font></dd></dl>
		<dl><dt>下单域名：</dt><dd><font face="arial"><?php echo $order_info['Order']['order_domain']?></font></dd></dl>
<?php //}?>
		<br />
	  </div>
	</div>
...
<!--TakeOver-->
	<div class="order_stat athe_infos tongxun orders_config">
	  ...
	  <div class="box">
		...
		<?php if(isset($all_address) && sizeof($all_address)>0){?>
		<?php foreach($all_address as $key=>$val){?>
		...
		<dl><dt>最佳送货时间：</dt><dd><input type="text" class="text_inputs" style="width:285px;" name="data[Order][best_time]" id="OrderBestTime" value="<?php echo $order_info['Order']['best_time']?>" /></dd></dl>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  <div class="box">
		...
		<dl><dt>发票类型：</dt><dd><input type="text" name="data[Order][invoice_type]" id="data_order_invoice_type" class="text_inputs" style="width:198px;" value="<?php echo $order_info['Order']['invoice_type']?>"/></dd></dl>
		...
		<dl><dt>缺货处理：</dt><dd><input type="text" id="data_order_how_oos" class="text_inputs" style="width:198px;" value="<?php echo $order_info['Order']['how_oos']?>" name="data[Order][how_oos]"/></dd></dl>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" class="products_infos">
  ...
  <input type="hidden" name="act_type" value="products">
	  <tr>
	  	<th width="17%" height="28" valign="middle">货号</th>
		<th width="11%" valign="middle">商品名称</th>
		...
		<th width="14%" valign="middle">操作</th>
	  	  ...
	  	  <?php if(isset($order_info['OrderProduct']) && sizeof($order_info['OrderProduct'])>0){?>
	<?php foreach($order_info['OrderProduct'] as $k=>$v){?>
	  <tr>
	  <tbody>
	    <input name="rec_id[]" type="hidden" value="<?php echo $v['id']?>" />
	  	<td width="17%" height="28" valign="middle"><?php echo $html->link($v['product_code'],$server_host.$cart_webroot."products/{$v['product_id']}",array('target'=>'_blank'));?></td>
		<td width="11%" valign="middle"><?php echo $v['product_name'];?><?php if(!empty($v['delivery_note'])){?><br />注：<?php echo $v['delivery_note'];}?></td>
		<td width="30%" valign="middle"><textarea name="product_attrbute[]" rows="3" style="height:auto;"><?php echo $v['product_attrbute']?></textarea></td>

		<td width="10%" valign="middle" align="center"><input type="text" name="product_price[]" value="<?php echo $v['product_price']?>" style="width:75px;text-align:right;" /></td>
		<td width="8%" valign="middle" align="center"><input type="text" id="product_quntity[]" name="product_quntity[]" value="<?php echo $v['product_quntity']?>" style="width:45px; text-align:right;" onblur="javascript:check_input(this);"/></td>
		<td width="10%" align="right" valign="middle"><?php echo $v['total']?></td>
		<td width="14%" valign="middle" class="handle" align="center"><?php if($order_info['Order']['shipping_status'] == 0){?><?php echo $html->link("删除","/orders/drop_order_products/{$v['id']}/{$order_info['Order']['id']}",'',false,false);?><?php }?></td>
	  </tbody></tr>
	<?php }}?>
<tr>
	  <tbody>
		<td valign="middle" colspan="4"><pre></pre></td>
		<td align="right" valign="middle"><strong>合计：</strong></td>
		<td valign="middle" align="right"><?php echo sprintf($price_format,sprintf("%01.2f",$order_info['Order']['subtotal']));?></td>
		<td valign="middle" align="center"><?php if($order_info['Order']['shipping_status'] == 0){?><input type="button" value="更新商品" onclick="update_order_product()" style="background:url(<?php echo $this->webroot?>img/upload.gif) no-repeat;width:78px;height:22px;border:0;font-size:12px;cursor:pointer" /><?php }?></td>
	  </tbody></tr>
...
<?php echo $form->create('',array('action'=>'edit_order_info','name'=>'UpdateOrder','onsubmit'=>'return false;'));?>
  ...
  <input type="hidden" name="product_id" id="product_id" />
	...
	<br />
	  ...
	  <input type="button" value="搜索" style="background:url(<?php echo $this->webroot?>img/upload.gif) no-repeat;width:78px;height:22px;border:0;font-size:12px;cursor:pointer;margin-top:-3px;vertical-align:middle;" onclick="searchProducts();"/>
  ...
  <input type="hidden" name="act_type" value="money"></>
	<div class="order_stat properies expend_infos">
	  ...
	  <div class="properies_left">
		...
		<dl><dt>使用余额：</dt><dd><dd><?php echo sprintf($price_format,sprintf("%01.2f",$balance_log["UserBalanceLog"]["amount"]));?><input type="hidden" name="balance" value="<?php echo $user_info['User']['balance']?>"></dd></dd>
		</dl>
		...
		<dl><dt>使用红包：</dt><dd>
			...
			<?php echo $order_info['Order']['coupon_fee']?>
	<?php if(isset($coupon_types_list) && sizeof($coupon_types_list)>0){?>
			<select id="coupon_fee_id">
			<option value="0">请选择...</option>
			<?php foreach( $coupon_types_list as $k=>$v ){?>
				<option value="<?php echo $v['CouponType']['id']?>" <?php if($v["CouponType"]["id"]==$coupon_info["Coupon"]["coupon_type_id"]){?>selected<?php }?>><?php echo $v["CouponTypeI18n"]["name"]?></option>
			<?php }?>
			</select>
	<?php }else{?>
			<select style="display:none" id="coupon_fee_id">
			<option value="0">请选择...</option>
			</select>
		<?}?>
			</dd></dl>
