 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: this_img_update.ctp 4717 2009-09-29 02:49:52Z zhengli $
*****************************************************************************/
?>
...
<?php echo $form->create('',array('action'=>'/',"type"=>"get",'name'=>"SearchForm"));?>
	...
	<input type="button"  value="选择更新" onclick="this_update_img_thumb_select()" />
	<input type="button"  value="全部更新" onclick="this_update_img_thumb_all()" />
</dl>
<?php echo $form->end();?>
...
<!--Main Start-->
<p class="add_categories"><strong><?php echo $html->link($html->image('add.gif',array('align'=>'absmiddle'))."下载图片到服务器","/product_image_batchs/",'',false,false);?></strong></p>
...
<script>
var thispage = "<?php echo $thispage+1;?>";
function product_search(){
	...
	window.location.href = webroot_dir+"product_image_batchs/this_img_update/"+product_code_value+"/"+keywords_value+"/";
}
</script>
<script tyep="text/javascript">
function this_update_img_thumb_select(){
	...
	var sUrl		=	webroot_dir+"product_image_batchs/this_update_img_thumb/"+product_id_str+"/";
	var request 	= 	YAHOO.util.Connect.asyncRequest('POST', sUrl, this_update_img_thumb_select_callback);
}
var this_update_img_thumb_select_Success = function(o){
...
var this_update_img_thumb_select_Failure = function(o){
	alert("error");
	YAHOO.example.container.wait.hide();
}
var this_update_img_thumb_select_callback ={
	success:this_update_img_thumb_select_Success,
	failure:this_update_img_thumb_select_Failure,
	timeout : 300000000,
	argument: {}
};

function this_update_img_thumb_all(){
	var product_code = GetId("product_code");
	var keywords = GetId("keywords");
	var product_code_value = product_code.value;
	var keywords_value = keywords.value;
	...
	window.location.href = webroot_dir+"product_image_batchs/this_img_update/"+product_code_value+"/"+keywords_value+"/"+"all/";
...
var update_database_img_Success = function(o){
	...
	if(product_id_arr.length>0){
		...
		window.location.href = webroot_dir+"product_image_batchs/this_img_update/"+product_code_value+"/"+keywords_value+"/all/?page="+thispage;
...
<?php if( $all_up_img == "all"&&$thispage+1<=$pagetotal){?>
