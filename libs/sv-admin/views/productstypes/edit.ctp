...
<!--Main Start--><p class="add_categories"><strong><?=$html->link($html->image('add.gif',array('align'=>'absmiddle'))."商品类型列表","/".$_SESSION['cart_back_url'],'',false,false);?></strong></p>
...
<input id="ProductTypeId" name="data[ProductType][id]" type="hidden" value="<?= $this->data['ProductType']['id'];?>"/>
...
<!--Menus_Config-->
	...
	<?php if(isset($languages) && sizeof($languages)>0)foreach($languages as $k => $v){?>
		...
		<input id="ProductTypeI18n<?=$k;?>ProductTypeId" name="data[ProductTypeI18n][<?=$k;?>][type_id]" type="hidden" value="<?= $this->data['ProductType']['id'];?>">
		<?=$html->image($v['Language']['img01'],array('align'=>'absmiddle'))?><span><input type="text" style="width:291px;border:1px solid #649776" id="name<?=$v['Language']['locale']?>" name="data[ProductTypeI18n][<?=$k;?>][name]" value="<?if(isset($this->data['ProductTypeI18n'][$v['Language']['locale']]))echo $this->data['ProductTypeI18n'][$v['Language']['locale']]['name'];?>" /></span> <font color="#F90071">*</font>
	</p><?php }?>
		</dd></dl>
