 ...
 * ===========================================================================
 * $开发: 上海实玮$
 * $Id: index.ctp 1172 2009-05-04 05:28:00Z shenyunfeng $
*****************************************************************************/
?>
...
<?if(isset($coupons) && sizeof($coupons)>0){?>
	...
	<li class="coupon_name_c" style="width:12%;"><span><?=$html->image('picflag.gif',array('align'=>'absmiddle'))?> <strong><?php echo $coupon['Coupon']['id']?></strong></span></li>
	...
	<li class="gratis" style="width:15%;">
			<?if($coupon['Coupon']['emailed'] == 0){?>
			未通知
			<?}else if($coupon['Coupon']['emailed'] == 1){?>
			已发成功
			<?}?>
	</li>
	...
	<?if($coupon['Coupon']['emailed'] == 0 && $coupon['Coupon']['order_id'] == 0){?>
	|
	...
	<?}?>
<?}?>
<?if($coupon_type != 0 && $coupon_type != 5){?>
	<ul class="product_llist memberlevels">
...
<?if(isset($coupons) && sizeof($coupons)>0){?>
	<?php foreach($coupons as $coupon){?>
	<ul class="product_llist memberlevels memberlevels_list coupons">
	<li class="coupon_name_c" style="width:15%;"><span><?=$html->image('picflag.gif',array('align'=>'absmiddle'))?> <strong><?php echo $coupon['Coupon']['id']?></strong></span></li>
