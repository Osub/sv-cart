...
<p class="add_categories"><strong><?php echo $html->link($html->image('add.gif',array('align'=>'absmiddle'))."服务商品列表","/".$_SESSION['cart_back_url'],'',false,false);?></strong></p>
...
<!--Main Start-->
<div class="home_main">
<?php echo $form->create('product_services',array('action'=>''.$this->data['Product']['id'],"OnSubmit"=>'return download_product_check();'));?>
...
<input type="hidden" name="ProductService[id]" value="<?php echo $downloads_info['ProductService']['id']?>" />
<input type="hidden" name="ProductService[product_id]" value="<?php echo $downloads_info['ProductService']['product_id']?>" />
...
<?php if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><input  id="down_name_<?php echo $v['Language']['locale']?>" name="data[ProductI18n][<?php echo $k;?>][name]" style="color:<?php echo $products_name_color;?>;width:215px;" type="text" maxlength="100"  <?php if(isset($this->data['ProductI18n'][$v['Language']['locale']])){?>value="<?php echo  $this->data['ProductI18n'][$v['Language']['locale']]['name'];?>"<?php }else{?>value=""<?php }?> /></span>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  其他信息</h1></div>
	  <div class="box">
		<dl><dt><?php echo $html->image('help_icon.gif',array('align'=>'absmiddle',"onclick"=>"help_show_or_hide('help_text')"))?>商品货号：</dt><dd><input type="text" class="text_inputs" id="ProductCode" name="data[Product][code]" onblur="update_code(this)" <?php if(isset($this->data['Product']['code'])){?>value="<?php echo  $this->data['Product']['code'];?>"<?php }else{?>value=""<?php }?>/><br /><span style="display:none" id="help_text2">如果您不输入商品货号，系统将自动生成一个唯一货号。</span></dd></dl>
...
<?php }}}}}}?>
  ...
  <?php if(!empty($this->data['other_cat'])){?>
		<?php foreach($this->data['other_cat'] as $k=>$v){?>
	 ...
	 <?php if(!empty($user_rank_list)){?>
		<dl>
		   ...
		   <?php foreach($user_rank_list as $k=>$v){?>
		   	   <p class="msg">
		<?php echo $v['UserRank']['name']?>
		<input class="text_inputs" style="width:50px;" id="rank_product_price<?php echo $k?>" name="rank_product_price[<?php echo $k?>]"  value="<?php echo @$v['UserRank']['product_price']?>" class="vmiddle" onkeyup="rank_product_price(<?php echo $k?>)" />
              <input type="hidden" id="user_rank<?php echo $k?>" name="user_rank[<?php echo $k?>]" value="<?php echo @$v['UserRank']['id']?>" /></p>
		       <p class="msg">折&nbsp;&nbsp;扣&nbsp;&nbsp;率 <input type="text" class="text_inputs vmiddle" style="width:50px;" id="user_price_discount<?php echo $k?>" name="user_price_discount[<?php echo $k?>]" value="<?php echo $v['UserRank']['discount']?>" onkeyup="set_price_note(<?php echo $k?>)" size="8" />
<input type="checkbox" class="checkbox" id="is_default_rank<?php echo $k?>" name="is_default_rank[<?php echo $k?>]" <?php if(@$v['UserRank']['is_default_rank']==1){ echo "checked"; }?> value="1" onclick="user_prince_check(this.checked,<?php echo $k?>)" >自动按照比例结算</p>
	...
	<?php }?>
	 <?php if(isset($SVConfigs['mlti_currency_module'])&&$SVConfigs['mlti_currency_module']==1){?>
		...
		<?php echo $v['Language']['name']?>: <input type="text"  class="text_inputs" style="width:30px;margin:-1px 0 0;padding:0;text-align:center"  name="locale_product_price[<?php echo $v['Language']['locale']?>][product_price]" value="<?php echo @$productlocaleprice_info[$v['Language']['locale']]['ProductLocalePrice']['product_price']?>"  size="8" />
		<input type="checkbox" name="locale_product_price[<?php echo $v['Language']['locale']?>][status]" value="1" <?php if(@$productlocaleprice_info[$v['Language']['locale']]['ProductLocalePrice']['status'] == 1){?>checked<?php }?>>是否有效
		<input type="hidden" name="locale_product_price[<?php echo $v['Language']['locale']?>][id]" value="<?php echo @$productlocaleprice_info[$v['Language']['locale']]['ProductLocalePrice']['id']?>" >
	...
	<?php }?>
		...
		<dl><dt><?php echo $html->image('help_icon.gif',array('align'=>'absmiddle',"onclick"=>"help_show_or_hide('help_text4')"))?>积分购买额度：</dt>
			<dd>
			<input type="text" class="text_inputs" name="data[Product][point_fee]" value="<?php echo @$this->data['Product']['point_fee'];?>"  />
			<p class="msg" style="display:none" id="help_text4">购买该商品时最多可以使用多少的积分</p></dd></dl>
		...
		<dl><dt>商品重量：</dt><dd><input type="text" class="text_inputs" value="<?php echo $this->data['Product']['products_weight_by_unit']?>" name="data[Product][weight]" id="ProductWeight" />
			...
			<dl style="*padding:4px 0;"><dt style="padding-top:1px">默认库存：</dt><dd class="best_input"><input class="text_inputs" value="<?php echo $this->data['Product']['quantity'];?>" name="data[Product][quantity]" id="ProductQuantity"/></dd></dl>
		...
		<dl style="*padding:4px 0;"><dt style="padding-top:1px">上架：</dt><dd class="best_input"><label for="ProductForsale"><input class="checkbox" type="checkbox" name="data[Product][forsale]" id="ProductForsale"value="1"<?php if($this->data['Product']['forsale'] == 1){?>checked<?php }?>/>打钩表示能作为普通商品销售，否则不允许销售。</label></dd></dl>
		<dl style="*padding:5px 0;"><dt style="padding-top:1px">能作为普通商品销售：</dt><dd class="best_input"><label for="ProductAlone"><input type="checkbox" class="checkbox" name="data[Product][alone]" id="ProductAlone"value="1"<?php if($this->data['Product']['alone'] == 1){?>checked<?php }?>/>打钩表示允许销售，否则只能作为配件或赠品销售。</label></dd></dl>
		<dl style="*padding:5px 0;"><dt style="padding-top:1px">是否开启下载：</dt><dd class="best_input"><input type="radio" name="ProductService[status]" value="1" <?php if ($downloads_info['ProductService']['status']==1){?>checked<?php }?>/>是&nbsp;&nbsp;<input type="radio" name="ProductService[status]" value="0" <?php if ($downloads_info['ProductService']['status']==0){?>checked<?php }?>/>否</dd></dl>
		<dl><dt>有效日期：</dt><dd class="time"><input style="width:90px;" class="text_inputs" type="text" id="date3" name="date3" value="<?php echo  $downloads_info['ProductService']['start_time']; ?>"  readonly/><?php echo $html->image("calendar.gif",array("id"=>"show3","class"=>"calendar_edit"))?>
			<input type="text" style="width:90px;" class="text_inputs" id="date4" name="date4"  value="<?php echo  $downloads_info['ProductService']['end_time']; ?>" readonly/><?php echo $html->image("calendar.gif",array("id"=>"show4","class"=>"calendar_edit"))?></dd></dl>	  
	  	<dl><dt>服务URL：</dt><dd><input type="text" class="text_inputs"style="width:228px;" name="ProductService[url]" value="<?php echo $downloads_info['ProductService']['url']?>"/></dd></dl>
	  	<dl><dt>服务周期：</dt><dd><input type="text" class="text_inputs"style="width:80px;" name="ProductService[service_cycle]" value="<?php echo $downloads_info['ProductService']['service_cycle']?>"/> 天</dd></dl>
...
<?php }else{?><?php echo $html->image("/..{$v['img_detail']}",'');?><?php }?></p>
		...
		<dd></dd>
		<dt>
			<?php echo $html->image($lv['Language']['img01'])?><input class="green_border" type="text" style='width:98%'  name="ProductGalleryI18ndescription[<?php echo $k?>][ProductGalleryI18n][<?php echo $lk?>][description]"   value="<?php echo @$v['ProductGalleryI18n'][$lv['Language']['locale']]['description']?>" id="ProductGallery<?php echo $k?>Description"/>
     ...
     <?php }?>
	  ...
	  <div id="Pro_img_div">
		...
		<span id="new_file"><span id="path_txt_id_1">上传文件: </span>
			...
			<input type="text" name="img_url[1]" id="upload_img_text_1" readonly/><?php echo $html->link($html->image('select_img.gif',array("class"=>"vmiddle icons","height"=>"20")),"javascript:img_sel(1,'products')",'',false,false)?>
			</span>
			</span>	</p>
			<p class="add_photo msg">图片描述:
<?php if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		  <?php echo $html->image($v['Language']['img01'],array("class"=>"vmiddle"))?>
		  <input type="text" name="img_desc[<?php echo $v['Language']['locale']?>][1]" id="img_desc[1]"/>
<?php }}?>
		排序: <input type="text" name="img_sort[1]" id="img_sort[1]" size="2"/></p>
<li>
