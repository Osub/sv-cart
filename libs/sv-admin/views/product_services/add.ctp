...
<?php echo $form->create('product_services',array('action'=>'add','name'=>'thisForm','id'=>'thisForm','OnSubmit'=>'return download_product_check();'));?>
		  <!--时间控件层start-->
...
<?php }}}}}}?>
	...
	<?php }?>
		...
		<dl style="*padding:5px 0;"><dt style="padding-top:1px">是否开启服务：</dt><dd class="best_input"><input type="radio" name="data[ProductService][status]" value="1" checked/>是&nbsp;&nbsp;<input type="radio" name="data[ProductService][status]" value="0" />否</dd></dl>
		<dl><dt>有效日期：</dt><dd class="time"><input style="width:90px;" class="text_inputs" type="text" id="date3" name="date3"   readonly/><?php echo $html->image("calendar.gif",array("id"=>"show3","class"=>"calendar_edit"))?>
	  	...
	  	<dl><dt>服务URL：</dt><dd><input type="text" class="text_inputs"style="width:228px;" name="data[ProductService][url]" /></dd></dl>
	  	<dl><dt>服务周期：</dt><dd><input type="text" class="text_inputs"style="width:80px;" name="data[ProductService][service_cycle]" /> 天</dd></dl>
...
<?php }else{?>
	<?php foreach( $shipping_list as $k=>$v ){?>
		<tr><td><?php echo $v["ShippingI18n"]["name"]?>:</td>
		<td><input type="text" style="width:120px;border:1px solid #649776" name="product_shoping_fee[<?php echo @$v['Shipping']['id']?>]" ></td><td>是否有效：<input type="checkbox" value="1" name="product_shoping_fee_status[<?php echo @$v['Shipping']['id']?>]" style="border:1px solid #649776"  /></td></tr>
	<?php }?>
<?php }?>
		</table>
		</div>
