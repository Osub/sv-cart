...
<?php echo $form->create('',array('action'=>'/',"type"=>"get",'name'=>"SearchForm"));?>
	...
	<?php }?>
	添加时间：<input type="text" id="date" name="date" value="<?php echo @$date?>"  readonly/>
	...
	有效时间：<input type="text" id="date3" name="date3" value="<?php echo @$date3?>"  readonly/>
...
<?php foreach($products_list as $k=>$v){?>	
	<tr>
		...
		|<?php echo $html->link("放入回收站","javascript:;",array("onclick"=>"layer_dialog_show('确定进回收站?','{$this->webroot}product_services/trash/{$v['Product']['id']}')"));?>
		</td>
	</tr>
<?php }}?>
 </table> 
...
//	window.location.href=webroot_dir+url;
	document.SearchForm.action=webroot_dir+"product_services/index/export";
	document.SearchForm.onsubmit= "";
	document.SearchForm.submit(); 
}
