...
<?php if(isset($languages) && sizeof($languages)>0){
	foreach ($languages as $k => $v){?>
		<p class="products_name"><?php echo $html->image($v['Language']['img01'])?><span><textarea name="data[CardI18n][<?php echo $k?>][description]"></textarea></span></p>
...
<!--Other Stat-->
	<div class="order_stat athe_infos tongxun">
	  ...
	  <div class="box">
	  <br />
		<dl><dt>上传图片1：</dt><dd><input type="text" size="35" style="font-size:12px;" id="upload_img_text_1" name="data[Card][img01]" /><br /><br /><?php echo @$html->image('',array('id'=>'logo_thumb_img_1','height'=>'150','style'=>'display:none'))?></dd><dd><?php echo $html->link($html->image('select_img.gif',$title_arr['select_img']),"javascript:img_sel(1,'cards')",'',false,false)?></dd></dl>
		
		<dl><dt>上传图片2：</dt><dd><input type="text" size="35" style="font-size:12px;" id="upload_img_text_2" name="data[Card][img02]" /><br /><br /><?php echo @$html->image('',array('id'=>'logo_thumb_img_2','height'=>'150','style'=>'display:none'))?></dd><dd><?php echo $html->link($html->image('select_img.gif',$title_arr['select_img']),"javascript:img_sel(2,'cards')",'',false,false)?></dd></dl>
